            ...
            <div className="space-y-3">
              <div className="relative">
                <select
                  ...
                  className="w-full p-4 rounded-2xl border-2 border-slate-100 bg-white hover:border-orange-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-200 outline-none transition-all font-display font-bold text-sm text-slate-700 appearance-none cursor-pointer"
                >
                  {FILM_VOICES.map((v) => (
                    <option key={v.id} value={v.id}>
                      {v.name} ({v.gender === 'NU' ? 'Nữ' : 'Nam'})
                    </option>
                  ))}
                </select>
              ...
              {selectedVoice && selectedVoice.id.startsWith('film_') && (
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-[10px] font-bold text-orange-700 uppercase tracking-wider">Đang chọn: {selectedVoice.name}</span>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${selectedVoice.gender === 'NU' ? 'bg-pink-100 text-pink-700' : 'bg-blue-100 text-blue-700'}`}>
            ...
            <div className="pt-4">
              ...
              <div className="relative">
                <select
                  value={voiceId}
                  onChange={(e) => setVoiceId(e.target.value)}
                  className="w-full p-3 rounded-xl border border-slate-200 bg-white hover:border-orange-200 focus:border-orange-500 outline-none transition-all font-bold text-xs text-slate-700 appearance-none cursor-pointer"
                >
