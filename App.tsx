...
import { Clapperboard, Sparkles, User, Type, Palette, Check, FileText, MessageCircle, Clock, Mic, Users, RotateCcw, MessageSquare, ArrowLeft, MapPin, Upload, Wand2, Loader2, PlusCircle, MinusCircle, ScanEye, Shirt, LogOut, Key, X, ExternalLink, Eye, EyeOff } from 'lucide-react';
          ...
          <div className="max-w-6xl mx-auto px-4 py-3 md:py-4 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center justify-between w-full md:w-auto">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h1 className="text-lg md:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-orange-600 to-amber-600">
                    NAM AI
                  </h1>
                  <p className="text-[10px] md:text-xs text-slate-500">098.102.8794</p>
                </div>
              </div>
            ...
            <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 w-full md:w-auto overflow-x-auto no-scrollbar">
              <button 
                onClick={() => setActiveModule('script')}
                className={`flex-1 md:flex-none whitespace-nowrap px-3 md:px-4 py-2 rounded-lg text-[11px] md:text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeModule === 'script' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                <Clapperboard size={14} /> Nhân Hóa
              </button>
              <button 
                onClick={() => setActiveModule('tts')}
                className={`flex-1 md:flex-none whitespace-nowrap px-3 md:px-4 py-2 rounded-lg text-[11px] md:text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeModule === 'tts' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                <Mic size={14} /> Giọng Đọc
              </button>
              <button 
                onClick={() => setActiveModule('vision')}
                className={`flex-1 md:flex-none whitespace-nowrap px-3 md:px-4 py-2 rounded-lg text-[11px] md:text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeModule === 'vision' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                <ScanEye size={14} /> Quét Ảnh
              </button>
              <button 
                onClick={() => setActiveModule('tryon')}
                className={`flex-1 md:flex-none whitespace-nowrap px-3 md:px-4 py-2 rounded-lg text-[11px] md:text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeModule === 'tryon' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                    ...
                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">API Key của bạn</label>
                      <div className="relative">
                        ...
                        <input 
                          ...
                          className="w-full pl-12 pr-12 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-orange-500 outline-none transition-all"
